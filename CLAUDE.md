...
The scriptable automation of that protocol lives under [`src/tools/live/`](src/tools/live/) (one module per API area plus a shared `harness`; Issues — including notes and discussions — Merge Requests, MR Discussions, Branches, and Repository Files domains so far), gated behind the `live-tests` cargo feature. Run with `cargo test --features live-tests` plus `GITLAB_URL`/`GITLAB_TOKEN` in the env; see [`docs/testing.md`](docs/testing.md) for details.
